...
test('(minimum, maximum) returns a Number at random with arguments. #1', t => {
  const MINIMUM = -50;
  const MAXIMUM = 50;
  ...
  for (let i = 0; i < TRIALS; ++i) {
    const result = getRandomDouble(MINIMUM, MAXIMUM);
...
test('(minimum, maximum) returns a Number at random with arguments. #2', t => {
  const MINIMUM = 0;
  const MAXIMUM = 100;
  const MEDIAN = (MAXIMUM - MINIMUM) / 2 + MINIMUM;
  const AMPLITUDE = (MAXIMUM - MINIMUM) / 2 * AMPLITUDE_MAGNIFICATION;
...
test('(minimum, maximum) returns a Number at random with arguments. #3', t => {
  const MINIMUM = -100;
  const MAXIMUM = -50;
  const MEDIAN = (MAXIMUM - MINIMUM) / 2 + MINIMUM;
  const AMPLITUDE = (MAXIMUM - MINIMUM) / 2 * AMPLITUDE_MAGNIFICATION;
